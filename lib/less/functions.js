...
function clearSrcVal(srcVal) {
    if (/^\[[^\]]+\]/.test(srcVal)) {
        return srcVal.substr(1, srcVal.length - 2);
    } else {
        return srcVal;
    }
}
...
tree.functions = {
    ...
    saturate: function (color, amount) {
        ...
        var r = hsla(hsl);
        if (color.srcVal) {
            r.srcVal = `saturate ${amount.value}${amount.unit.numerator[0]} | ${clearSrcVal(color.srcVal)}`;
        }
        return r;
    },
    desaturate: function (color, amount) {
        ...
        var r = hsla(hsl);
        if (color.srcVal) {
            r.srcVal = `desaturate ${amount.value}${amount.unit.numerator[0]} | ${clearSrcVal(color.srcVal)}`;
        }
        return r;
    },
    lighten: function (color, amount) {
        ...
        var r = hsla(hsl);
        if (color.srcVal) {
            r.srcVal = `lighten ${amount.value}${amount.unit.numerator[0]} | ${clearSrcVal(color.srcVal)}`;
        }
        return r;
    },
    darken: function (color, amount) {
        ...
        var r = hsla(hsl);
        if (color.srcVal) {
            r.srcVal = `darken ${amount.value}${amount.unit.numerator[0]} | ${clearSrcVal(color.srcVal)}`;
        }
    ...
    fadein: function (color, amount) {
        ...
        var r = hsla(hsl);
        if (color.srcVal) {
            r.srcVal = `fadein ${amount.value}${amount.unit.numerator[0]} | ${clearSrcVal(color.srcVal)}`;
        }
        return r;
    },
    fadeout: function (color, amount) {
        ...
        var r = hsla(hsl);
        if (color.srcVal) {
            r.srcVal = `fadeout ${amount.value}${amount.unit.numerator[0]} | ${clearSrcVal(color.srcVal)}`;
        }
        return r;
    },
    fade: function (color, amount) {
        ...
        var r = hsla(hsl);
        if (color.srcVal) {
            r.srcVal = `fade ${amount.value}${amount.unit.numerator[0]} | ${clearSrcVal(color.srcVal)}`;
        }
        return r;
    },
    spin: function (color, amount) {
        ...
        var r = hsla(hsl);
        if (color.srcVal) {
            r.srcVal = `spin ${amount.value}${amount.unit.numerator[0]} | ${clearSrcVal(color.srcVal)}`;
        }
        return r;
    },
    //
    ...
    color: function (n) {
        if (n instanceof tree.Quoted) {
            ...
            else if (/site_\d/.test(n.value)) {
                // alexahdp
                n.srcVal = n.value;
                n.value = colorCandidate = '#fff';
                return new(tree.Color)(colorCandidate.slice(1));
            }
